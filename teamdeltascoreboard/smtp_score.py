...
def score_SMTP(queue, alive, lock, target, port=DPORT, value=1, team=0):
    mail_server = SMTP()
    while alive():
        try:
            ...
            lock.acquire()
            queue.put({'service': 'SMTP', 'status': 'UP', 'host' : target, 'value':value, 'team': team})
            lock.release()
        except Exception: # If any exception is thrown, it's the result of the server being down or misconfigured
            lock.acquire()
            queue.put({'service': 'SMTP', 'status': 'DOWN', 'host' : target, 'value':value, 'team': team})
            lock.release()
        sleep(300)